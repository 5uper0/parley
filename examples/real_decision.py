...
def collect_ratifications(ask, say, parties: list, result) -> list:
    ...
    for party in parties:
        ...
        try:
            acceptances.append(ratify(sheet, result.transcript, accept=accept))
        except Exception as exc:
            say(f"Refused: {exc}")
            acceptances.append(ratify(sheet, result.transcript, accept=False))
        ask("  press enter ")
    say(CLEAR)
    return acceptances
...
def main(argv=None) -> int:
    ...
    if result.status == "deadlock":
        ...
        acceptances = collect_ratifications(ask, say, parties, result)
